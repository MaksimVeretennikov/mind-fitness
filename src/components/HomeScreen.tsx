  ...
  return (
    <div data-home="true" className="min-h-screen px-4 py-10 animate-fade-in">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Geography / Кругозор Section */}
        <div className="mt-14">
          <div className="glass rounded-2xl shadow-sm p-6 mb-7">
            <h2 className="text-2xl font-bold text-gray-800 mb-1">Кругозор 🌍</h2>
            <p className="text-gray-500 text-sm font-medium section-subtitle">География, история, наука</p>
            <div className="mt-3 h-px bg-gradient-to-r from-teal-500/40 via-emerald-400/25 to-transparent" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {GEOGRAPHY_EXERCISES.map((ex, i) => (
              <div
                key={ex.id}
                className="glass rounded-2xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group flex flex-col"
                style={{ animationDelay: `${(EXERCISES.length + RUSSIAN_EXERCISES.length + i) * 60}ms` }}
                onClick={() => onSelect(ex.id)}
              >
                <div
                  className={`inline-flex items-center justify-center w-14 h-14 rounded-xl bg-gradient-to-br ${ex.gradient} shadow-md mb-4 group-hover:scale-110 transition-transform duration-300`}
                ...
                <h2 className="text-lg font-semibold text-gray-800 mb-2">{ex.title}</h2>
                <p className="text-gray-500 text-sm leading-relaxed mb-5 flex-1">{ex.description}</p>
