...
Данные: [introWordsData.ts](src/data/introWordsData.ts) (~65 вводных + ~51 не вводное, всего ~116 слов).

**Механика:** пользователь видит все N слов сразу (карточки, flex-wrap), на каждой карточке
две кнопки: «Вводное» / «Не вводное». Выбор не раскрывается до нажатия «Проверить результат».
Результат показывается только в конце (зелёные/красные карточки + стрелка с верным ответом).

- Настройка: 10 / 20 / 30 слов. Гарантированно не менее 3 слов каждого вида.
- Пара слов имеет контекстную заметку (e.g. «=следовательно»), отображаемую курсивом.
- Хранение: `exercise_results`, `resultKey = 'intro-words'`, `ruScore = correct * 10`.
  Формат ошибок: `{ display, chosen, correct }` — «вводное» / «не вводное».
- История ошибок: `MistakesHistory exerciseName="intro-words"`.
- Работа над ошибками: `ErrorDrill mode="choice"`.
